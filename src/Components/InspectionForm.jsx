...
const InspectionForm = () => {
  ...
  const handleFormSubmit = async (val) => {
    ...
    const form = {
      ...
      license: val.license,
      email: val.email,
      serviceDate: new Date(val.serviceDate).getTime(),
      servicedAt: val.servicedAt,
      nextService: new Date(val.nextService).getTime(),
    ...
    const result = await fetch(`${baseUrl}/inspection/addNewInspection`, {
      method: "POST",
      headers: {
  ...
  return (
    <>
      ...
      <Formik initialValues={initialValues} validationSchema={inspectionSchema}>
        {({ values, errors, touched, handleBlur, handleChange }) => (
          ...
          >
            <Grid container spacing={2}>
              ...
              <InspectionFormBox
                data={interiorExteriorFields}
                handleChange={handleChanges}
                section={"interiorExteriorFields"}
              />
              <InspectionFormBox
                data={batteryFields}
                handleChange={handleChanges}
                section={"batteryFields"}
              />
              <InspectionFormBox
                data={underHoodFields}
                handleChange={handleChanges}
                section={"underHoodFields"}
              />
              <InspectionFormBox
                data={underVehicleFields}
                handleChange={handleChanges}
                section={"underVehicleFields"}
              />
              <Grid item xs={12} className="mt-8">
                <div className="min-w-full border border-black dark:border-white border-2">
                  <Grid item xs={12} className="">
                    <h1 className="font-breulGroteskBBold text-[40px] justify-center items-center text-center text-white dark:text-black bg-black dark:bg-white">
                      BRAKES
                    </h1>
                  </Grid>
                  <Grid item xs={12} className="">
                    <h1 className="font-breulGroteskBBold text-[40px] justify-center items-center text-center text-black dark:text-white">
                      BRAKE PADS - SHOES
                    </h1>
                  </Grid>
                </div>
                <div className="border border-black dark:border-white border-2">
                  <Grid item xs={12} className="flex flex-row items-center">
                    ...
                    <h1 className="font-breulGroteskBBold text-[20px] justify-center items-center text-center text-black dark:text-white">
                      {"Over 5mm (Disk) or 2mm (Drum)"}
                    </h1>
                  </Grid>
                  <Grid item xs={12} className="flex flex-row items-center">
                    <div className="p-4 m-2 border border-black border-2 bg-yellow-500"></div>
                    <h1 className="font-breulGroteskBBold text-[20px] justify-center items-center text-center text-black dark:text-white">
                      {"3.5mm (Disk) or 1.01-2mm (Drum)"}
                    </h1>
                  </Grid>
                  <Grid item xs={12} className="flex flex-row items-center">
                    <div className="p-4 m-2 border border-black border-2 bg-red-500"></div>
                    <h1 className="font-breulGroteskBBold text-[20px] justify-center items-center text-center text-black dark:text-white">
                      {"Less than 3mm (Disk) or 1mm (Drum)"}
                    </h1>
                  </Grid>
                </div>
                <FieldsMap
              ...
              <Grid item xs={12} className="mt-8">
                ...
                <RichTextEditor
                  content={preInspectionReport}
                  setContent={setPreInspectionReport}
                  themeMode={themeMode}
                />
              </Grid>
              <Grid item xs={12} className="mt-8">
              ...
              <Grid item xs={12} className="mt-8">
                ...
                <RichTextEditor
                  content={postInspectionReport}
                  setContent={setPostInspectionReport}
                  themeMode={themeMode}
                />
              </Grid>
            </Grid>
    ...
    <div className="flex gap-4 items-center justify-center mt-8 font-qanelasRegular text-black dark:text-white">
      {options.map((option) => (
        <label key={option.id} className="flex items-center cursor-pointer">
          ...
          <span className={`ml-2 ${option.id === selectedOption ? "" : ""}`}>
            {option.label}
          </span>
        </label>
      ))}
